...
Proof.
  intros n. split.
  - intros E. induction E as [| | n' m' Hn' Hn'' Hm' Hm''].
    { apply ev_0. }
    { apply ev_SS. apply ev_0. }
    { apply ev_sum. apply Hn''. apply Hm''. }
  - intros E. induction E as [| n' Hn' E].
    { apply even'_0. }
    { assert(H: 2 + n' = S(S n')).
      { reflexivity. }
      rewrite <- H.
...
Proof.
  intros n m Hnm Hn. generalize dependent m. induction Hn as [| n' E' H'].
  - intros m H. simpl in H. apply H.
  - intros m H. apply H'. simpl in H.
...
Inductive total_relation: nat -> nat -> Prop:=
  ...
  | total_n_m n m (H: total_relation n m): total_relation (S n) (S m).
(* [] *)
...
Inductive _eq: nat -> nat -> Prop:=
  | _eq_n_m n m (H: _eq n m): _eq (S n) (S m).
...
Lemma Sn_le_Sm__n_le_Sm: forall n m,
  S n <= m -> n <=  m.
Proof.
  intros n m H.
  ...
  - intros n H.
    inversion H.
  - intros n H. apply le_S. inversion H.
    { apply le_n. }
    { apply IHm. apply H1. }
Qed.
...
Proof.
  ...
  - assert(H': S n1 <= S (n1 + n2)).
    ...
    Search "le_trans". apply le_trans with (n:= S (n1 + n2)).
    { apply H'. }
    { apply H. }
  - assert(H': S n2 <= S (n1 + n2)).
...
(* leb_complete : forall n m, n <=? m = true -> n <= m.*)
...
(** - Which of the following propositions are provable?
    ...
    - If we dropped constructor [c5] from the definition of [R],
      ...
      Yes. Cannot derive the rule c5

    - If we dropped constructor [c4] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer.
      Yes. Can derive the rule c4
...
Lemma fR_then_R: forall m n o,  fR m n = o -> R m n o.
Proof.
  intros m n o.
  generalize dependent n.
  ...
  - induction o.
    ...
    { intros n. simpl. intros H. rewrite <- H. apply R_0_n_n. }
  - intros o n H.
    apply c4. apply c2. apply c2.
    apply IHm.
    rewrite plus_comm.
    simpl in H.
    simpl.
...
Inductive subseq : list nat -> list nat -> Prop :=
  | subseq1: subseq [] []
  | subseq2 xs: subseq [] xs
  | subseq3 xs ys (H: subseq xs ys): forall (n:nat), subseq (n:: xs) (n :: ys).

Theorem subseq_refl : forall (l : list nat), subseq l l.
Proof.
  induction l as [| x' l' IHl].
  - apply subseq1.
  - apply subseq3. apply IHl.
Qed.

Lemma head_app: forall (X:Type) (x:X) (l: list X),
  x :: l= [x] ++ l.
Proof.
  simpl.
  reflexivity.
...
Lemma head_append: forall (X:Type) (x:X) (l1 l2:list X),
  x :: l1 ++ l2 = (x :: l1) ++ l2.
Proof.
  intros X x l1 l2. 
  rewrite -> (head_app X  x l1).
  rewrite head_app.
  apply app_assoc.
  Qed.
...
Proof.
  induction l1.
  - induction l2.
    { intros l3 H1 H2. apply subseq2. }
    { intros l3 H1 H2. apply subseq2. }
  - induction l2.
    { intros l3 H1 H2. inversion H1. }
    { intros l3 H1 H2. 
      induction l3.
      { inversion H2. }
...
Proof.
  ...
  - simpl. apply MStarApp.
    ...
    { apply IHss. intros s Hin. 
      apply H. simpl. right. apply Hin.
    }
Qed. 
(** [] *)
...
Lemma  app_empty: forall (X:Type) (xs ys: list X), xs ++ ys = [ ] -> xs = [] /\ ys = [].
Proof.
  intros X xs ys H.
  induction xs.
  - split.
...
Lemma add_head: forall (X:Type) (x: X) (xs ys: list X),
  xs = ys -> x:: xs = x :: ys.
Proof.
  intros X x xs ys H.
  rewrite H.
...
Proof.
  ...
  - intros H.
    ...
    { intros s1 H. induction s1.
      { simpl in H.
        inversion H.
        apply app_empty in H1.
        destruct H1 as [Hs1 Hs0].
        inversion H3.
        rewrite -> Hs1 in H1.
        inversion H1.
      }
...
Proof.
  ...
  - simpl. rewrite In_app_iff in *.
    right.
    apply (IH Hin).
  - simpl. simpl in Hin. exfalso. apply Hin.
...
- intros H.
  ...
  { simpl in H.  rewrite andb_true_iff in H.
    ...
    apply IHre1 in Hre1 as IHre1'.
    apply IHre2 in Hre2 as IHre2'.
    destruct IHre1 as [s1 IHre1'']. apply Hre1.
    destruct IHre2 as [s2 IHre2'']. apply Hre2.
    exists (s1 ++ s2).
  ...
  { simpl in H. rewrite orb_true_iff in H.
    ...
    { apply IHre1 in Hre as Hre'.
      destruct Hre' as [s Hre''].
      exists s.
      apply (MUnionL s re1 re2).
      apply Hre''.
    }
    { apply IHre2 in Hre as Hre'.
      destruct Hre' as [s Hre''].
      exists s.
      apply (MUnionR re1 s re2).
      apply Hre''.
    }
  }
  { exists []. apply MStar0. 
...
Lemma MStar'' : forall T (s : list T) (re : reg_exp),
  s =~ Star re -> exists ss : list (list T), s = fold app ss [] /\ forall s', In s' ss -> s' =~ re.
Proof.
  intros T s re H.
  remember (Star re) as re'.
  induction H.
  ...
  - exists [s1;s2].
    ...
    { intros s' H'.
      ...
      (* s1 = s' *)      
      { injection Heqre' as Hre.
        rewrite Hre in H. 
        rewrite H1' in H.
        apply H.
      }
      (* s2 = s' *) 
      { apply IHexp_match2 in Heqre' as H'.
        destruct H' as [ss [Hss1 Hss2]].
Admitted.
...
Proof.
  ...
  - split.
    ...
    { intros TT. apply H. }
  - split.
    { intros Hp. apply H in Hp. exfalso. apply Hp. }
...
Proof.
  ...
  - simpl in *.  destruct (eqbP n x) as [H' | H'].
    { simpl in H. discriminate H. }
    { simpl in H. intros H''. apply IHl.
      { apply H. }
      { destruct H'' as [Hl| Hr].
        { rewrite Hl in H'. exfalso. apply H'. reflexivity. }
        {  apply Hr. }
      }
    } 
Qed.
