...
Proof. induction n as [|n' IHn'].
- simpl. reflexivity.
- simpl. intro n. rewrite -> IHn'. reflexivity.
Qed. 

Theorem plus_comm : forall n m : nat,
  n + m = m + n.
Proof. induction n as [|n' IHn'].
- simpl. intro m. rewrite <- plus_n_O. reflexivity.
- simpl. intro m. rewrite <- plus_n_Sm. rewrite <- IHn'. reflexivity.
Qed.
...
Proof. induction n as [|n' IHn'].
- simpl. reflexivity.
- rewrite -> IHn'. simpl. rewrite -> Basics.negb_involutive. reflexivity.
Qed.
(** [] *)
...
(* FILL IN HERE 
  With induction we have a hypothesis when in the induction step whereas with 
  destruct we wouldn't.
*)
*)
...
(** **** Exercise: 2 stars, advanced, recommended (plus_comm_informal)  
    ...
    Base Case: n = 0  
      We want to show that 0 + m = m + 0.
      Since 0 + m = m by definition and m + 0 = m from plus_n_0, we have
      0 + m = 0 = m + 0. Thus the base case is true. 
    
    Induction Step
      Suppose n + m = m + n for some natural nummber n. We want to deduce that
      S(n) + m = m + S(n). Since S(n) + m = S(n + m) by definition and m + S(n) = S(n + m) from plus_n_Sm
      we have S(n) + m = S (n + m) = m + S(n). Thus, we have deduced S (n) + m = m + S(n).
    
    Hence from the principle of induction, n + m = m + n for all natural numbers n and m.  
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)  
    ...
    Base Case: n = 0
      By defintion 0 ?= 0 = true. Thus the base case is true.
    
    Induction Step: 
      Suppose n=?n = true, for some n. By definition
      (S n =? S n) = (n =? n) = true. 

    Hence, from the principle of induction, n =? n for all natural numbers n. 
...
Lemma mult_Sm_n: forall m n:nat,n * S m  = n + n * m .
Proof. intro m. induction n as [|n' IHn'].
- simpl. reflexivity.
...
- simpl. rewrite -> mult_Sm_n. rewrite -> IHn'. reflexivity.
Qed.
(** [] *)
...
- simpl. intro H. apply IHp' in H. rewrite -> H. reflexivity.
Qed.
...
Theorem S_nbeq_0 : forall n:nat,
  (S n) =? 0 = false.
Proof. intro n. simpl. reflexivity.
Qed.
  

Theorem mult_1_l : forall n:nat, 1 * n = n.
Proof. intros n. simpl. rewrite <- plus_n_O. reflexivity.
Qed.

Theorem all3_spec : forall b c : bool,
    orb (andb b c) (orb (negb b) (negb c)) = true.
Proof. intros b c. destruct b eqn:Eqb. destruct c eqn:Eqc.
- simpl. reflexivity.
- simpl. reflexivity.
- simpl. reflexivity.
Qed.
...
Lemma incr_b_succ_b: forall b, bin_to_nat(incr b) = S (bin_to_nat b).
Proof. induction b as [|b' IHb'| b'' IHb''].
- simpl. reflexivity.
- simpl. reflexivity.
- simpl. rewrite -> IHb''.
  simpl. 
...
(*
Lemma bin_to_nat_incr_b: forall b,
  bin_to_nat(incr b) = S (bin_to_nat b).
Proof. induction b as [ | b' IHb'|  b'' IHb''].
- simpl. reflexivity.
- simpl. reflexivity.
- simpl.
  assert (H1: bin_to_nat b'' + 0 = bin_to_nat b'').
    {  rewrite <- plus_n_O. reflexivity. }
  rewrite -> H1.
  assert (H2: bin_to_nat (incr b'') + 0 = bin_to_nat (incr b'')).
    {  rewrite <- plus_n_O. reflexivity. }
  rewrite -> H2.
  rewrite -> incr_b_succ_b.
  simpl.
  rewrite <- plus_n_Sm.
  reflexivity.
Qed.
