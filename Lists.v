...
Proof. destruct p as [n m]. simpl. reflexivity. Qed.
(** [] *)
...
Fixpoint nonzeros (ls:natlist) : natlist:=
  match ls with
  | nil => nil
  | l :: ls' => match l with
                | O => nonzeros ls'
...
Fixpoint oddmembers (ls:natlist) : natlist :=
  match ls with
  | nil => nil
  | l :: ls' => match oddb l with
                | false => oddmembers ls'
                | true => l :: (oddmembers ls')
                end
  end.
...
Fixpoint alternate (xs ys : natlist) : natlist:=
  match xs, ys with
  | nil, _  => ys
  | _ , nil => xs
...
Fixpoint count (v:nat) (xs:bag) : nat:=
  match xs with
  | nil => O
  | x :: xs' => match x =? v with
                 | false => count v xs'
...
Fixpoint remove_one (v:nat) (xs:bag) : bag:=
  match xs with
  | nil => nil
  | x :: xs' => match x =? v with
                | true => xs'
...
Fixpoint remove_all (v:nat) (xs:bag) : bag :=
  match xs with
  | nil      => nil
  | x :: xs' => match  x =? v with
...
Fixpoint subset (xs:bag) (ys:bag) : bool :=
  match xs with
  | nil => true
  | x :: xs' => match member x ys with
                | false => false
                | true => subset xs' (remove_one x ys)
                end
  end.
...
(*
Theorem bag_theorem : forall (n:nat), forall (vs: bag), count n (add n vs) = S (count n vs) .
Proof. simpl. induction vs as [|v' vs' IHvs'].
...
Theorem app_nil_r : forall l : natlist,
  l ++ [] = l.
Proof. induction l as [|n ls' IHls'].
-  simpl. reflexivity.
...
Proof. induction l1 as [| n l1' IHl1']. 
- intros l2 l3 l4. simpl.   
  rewrite -> app_assoc.
  reflexivity.
- intros l2 l3 l4. simpl. 
  rewrite <- IHl1'. 
...
-  intros l2.  induction n as [|n IHn'].
  +  simpl. rewrite -> IHl1'. reflexivity.
  +  simpl. rewrite -> IHl1'. reflexivity.
Qed.
(** [] *)
...
Fixpoint eqblist (xs ys : natlist) : bool:=
  match xs, ys with
  | nil, nil => true
  | nil, _   => false
  | _  , nil => false
...
Theorem eq_or_neq: forall (n1 n2:nat), (n1 =? n2 = true) \/ (n1 =? n2 = false).
Proof. induction n1 as [|n1' IHn1'].
- induction n2 as [|n2' IHn2'].
  +  simpl. left. reflexivity.
  +  simpl. right. reflexivity.
-  induction n2 as [|n2 IHn2'].
  + simpl. right. reflexivity.
  + simpl. destruct (IHn1' n2) as [eq| neq].
  { rewrite -> eq. left. reflexivity. }  
  { rewrite -> neq. right. reflexivity. }
Qed.

Theorem count_sum: forall (n: nat), forall (xs ys: bag), 
  count n xs + count n ys = count n (sum xs ys). 
Proof. induction n as [|n' IHn'].
  -  induction xs as [|x xs' IHxs'].
    + simpl. reflexivity.
    + simpl. destruct x as [| x'] eqn: Eqx'.
      { simpl. intros ys. rewrite -> IHxs'. reflexivity. }
      { simpl. intros ys. rewrite -> IHxs'. reflexivity. }
  - induction xs as [| x xs' IHxs'].
    + simpl. reflexivity.
    + simpl. destruct (eq_or_neq x (S n')).
      { rewrite -> H. intros ys. rewrite <- IHxs'. simpl. reflexivity. }
      { rewrite -> H. intros ys. rewrite <- IHxs'. reflexivity. }
Qed.
...
Theorem rev_rev: forall (xs:natlist), rev(rev xs) = xs.
Proof. induction xs as [|x xs'].
- simpl. reflexivity.
- simpl. rewrite -> rev_app_distr. rewrite -> IHxs'. simpl. reflexivity.
Qed.

Theorem rev_eq: forall (xs ys : natlist), rev xs = rev ys -> xs = ys.
Proof. intros xs ys H . 
  assert (H1: rev (rev xs) = xs).
  { rewrite -> rev_rev. reflexivity. }
  assert (H2: rev (rev ys) = ys).
  { rewrite -> rev_rev. reflexivity. }
...
Proof. intro x. destruct x eqn:Eqx. induction n as [|n IHn'].
- simpl. reflexivity.
...
Proof. induction d as [|d' IHd'].
- simpl. intros x v. rewrite <- eqb_id_refl. reflexivity.
...
Proof. induction d as [|d' IHd'].
- intros x y o. intros H. simpl. rewrite -> H. reflexivity.
- intros x y o. intros H. simpl. rewrite -> H. reflexivity.
Qed.
(** [] *)
End PartialMap.
...
The type baz has infinite many elements.     
