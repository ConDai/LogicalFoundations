...
- destruct m.
  { reflexivity. }
  { symmetry. rewrite -> plus_comm in Hnm. discriminate Hnm. }
Qed.
...
Proof.
  (* WORKED IN CLASS *)
  unfold not. intros P Q [HP nHP]. apply nHP in HP as contra.
  destruct contra.
Qed.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  unfold not. intros P Q Hpq Hqf Hp.
...
Proof.
  ...
  - intros H.
    destruct H as [ePx | eQx].
    { destruct ePx as [x Px].
      exists x.
      left.
      apply Px.
    }
    { destruct eQx as [x Qx].
      exists x.
      right.
      apply Qx.
    }
...
Lemma and_distributes_over_or: forall P Q R : Prop,
  P /\ (Q \/ R) <-> (P /\ Q) \/ (P /\ R).
Proof.
  intros P Q R.
  split.
...
Proof.
  ...
  - induction l as [|x l' IHl].
    { simpl. reflexivity. }
    { intros H.
      simpl. simpl in H. simpl in IHl.
      split.
      { apply H. left. reflexivity. }
      { apply IHl.  intros x0. intros H'. 
        apply H.
        right.
        apply H'.
      }
  ...
  - induction l as [|x l' IHl].
    { simpl. intros H x []. }
    { simpl. intros [H1 H2]. intros x0 [H1' | H2'].
      { rewrite <- H1'. apply H1. }
      { apply IHl. 
        { apply H2. }
        {  apply H2'. }
...
Theorem combine_odd_even_intro :
  forall (Podd Peven : nat -> Prop) (n : nat),
    (oddb n = true -> Podd n) -> (oddb n = false -> Peven n) -> combine_odd_even Podd Peven n.
Proof.
  intros Podd Peven n Hodd Heven.  unfold combine_odd_even. destruct (oddb n).
...
Proof.
  ...
  - intros H Htt. apply H.
  - intros H Htf. discriminate Htf.
Qed.
...
Theorem rev_rev: forall (X:Type) (xs: list X), rev(rev xs) = xs.
Proof. induction xs as [|x xs'].
- simpl. reflexivity.
- simpl. rewrite -> rev_app_distr. rewrite -> IHxs'. simpl. reflexivity.
Qed.

Lemma tr_rev_empty: forall (X:Type) (xs ys: list X),
  rev_append xs ys = rev_append xs [] ++ ys.
Proof.
  intros X. induction xs.
  - simpl. reflexivity.
  - intros ys . simpl.  
    rewrite -> IHxs with (ys:= x::ys).
    rewrite -> IHxs with (ys := [x]).
    Search "app_assoc".
    rewrite   <- app_assoc.
    simpl.
    reflexivity.
...
Proof.
  ...
  intro x. unfold tr_rev. induction x.
  - simpl. reflexivity. 

  - simpl. 
    rewrite tr_rev_empty.
    rewrite IHx.
    reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct b1. 
    ...
    { destruct b2.  
      { simpl. intros [H1 H2]. discriminate H1. }
      { simpl. intros [H1 H2]. discriminate H1. }
    }
Qed.
...
(* eqb_eq: forall n1 n2 : nat, (n1 =? n2) = true <-> n1 = n2 *)
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool) (l1 l2 : list A) : bool:=
  ...
  | [], _   => false
  | _,  []  => false
  | x1 :: l1', x2 :: l2' => if eqb x1 x2 then eqb_list eqb l1' l2' else false
  end.
                            
Lemma list_eq: forall A (x y: A) (xs ys: list A),
  x = y /\ xs = ys -> x::xs = y :: ys.
Proof.
  intros A x y xs ys [Hxy Hxsys].
  rewrite Hxy. rewrite Hxsys.
  reflexivity.
Qed.
  ...
  { destruct l2.
    ...
    { simpl. split.
      { intros H'. 
        { apply list_eq.
          split.
          { apply H. destruct (f x x0).
            { reflexivity. }
            { discriminate H'. }
          }
          { apply IHl1. destruct (f x x0).
            { apply H'. }
            { discriminate H'. }
          }
        }
...
Proof.
  ...
  - split.
    { simpl. intros H'. 
      ...
      destruct H' as [Htest Hforall].
      split.
      { apply Htest. }
      { apply IHl. apply Hforall. }
    }
    { simpl. intros [Htestx Hforall]. 
      apply andb_true_iff.
      split.
      { apply Htestx. }
      { apply IHl. apply Hforall. }
...
Proof.
  ...
  unfold not in exm.
  intros x.
  assert (H':  P x \/ (P x -> False)).
    { apply exm. }
  destruct H' as [isP | notP].
  - apply isP.
  - exfalso.
    apply H.
    exists x.
