...
(* Rewrite is used when rewriting a term using equalities in a context 
  or equalities from previous Lemmas. Apply on the other hand is used on
  goals or goals with implications. We can end the proof using apply but 
  not rewrite.
    [] *)
...
Proof. 
  intros X x y z i j H1 H2. 
  injection H2. intros Hij Hyx. 
  symmetry. apply Hyx.
Qed.
 (** [] *)
...
Proof. 
  intros A B. intros f x y. intros H. 
  rewrite -> H.
  reflexivity.
Qed.
(* ################################################################# *)
...
Lemma eq_succ: forall n m, n = m -> S n = S m.
...
- induction m as [|m IHm'].
  ...
  { intros H1.
    simpl in H1.
    injection H1.
    assert(HnSn: n' + S n' = S (n' + n')).
      { rewrite -> plus_n_Sm. reflexivity. }
    assert(HmSm: m + S m = S (m + m)).
      { rewrite -> plus_n_Sm. reflexivity. }
    intros H2.
    rewrite -> HnSn in H2.
    rewrite -> HmSm in H2.
...
-  intros n. induction n as [|n' IHn'].
  ...
  { intros H. simpl in H. simpl. 
     apply IHl'. injection H. 
     intros goal. apply goal.
  }
Qed.
...
Proof. 
  ...
  destruct b.
  (* b = true *)
  { destruct (f true) as [|] eqn:H0. 
  (* f true = true *)    
  { rewrite -> H0. rewrite -> H0. reflexivity. }
    { destruct (f false) as [|] eqn: H1. 
      (* f false = true*)     
      { apply H0. }
      (* f false = false *)      
      { apply H1. }
  ...
  (* b = false *)
  { destruct (f false) as [|] eqn:H0.
    (* f false = true*)    
    { destruct (f true) as [|] eqn:H1.
      (* f true = true *)   
      { apply H1. }
      (* f true = false *)
      { apply H0. }
...
Theorem eqb_true':  forall n m : nat, n = m  ->  (n =? m) = true.
...
Proof.
  ...
  - induction l2.
    ...
    { simpl. intros H. injection H.
      intros H'. apply IHl1 in H'. rewrite -> H'.
      simpl. reflexivity.
    }
Qed.
